...
<main class="p-4 h-full">
        ...
        <div class="flex">
            {#each projects as project}
                <div class="relative w-96 flex flex-col overflow-hidden m-1 last:mr-32">
                    ...
                    <p class="absolute border-2 p-2 text-black bg-white dark:text-white dark:bg-black transition-all overflow-hidden" style:bottom={descShown ? '0' : '-100px'}>
                        {project.description}
                    </p>
                </div>
            {/each}
    ...
    <footer>
        <a href="https://github.com/armandmgt/armandmgt.github.io" class="text-xs">
            <GithubIcon size="12" class="inline mr-1"/>See on Github
        </a>
    </footer>
</main>
